...
\[
\sum _{i \in I} \vert x_i H\vert = \vert G\vert \implies [G:H] \vert H\vert = \vert G\vert.
\]
...
Também deduzimos da demonstração anterior do \hyperlink{lagrange}{\textbf{Teorema de Lagrange}} que:

\[
\text{Card}(xH) = \vert H\vert.
\]
...
Ora, basta aplicar sucessivamente o \hyperlink{lagrange}{\textbf{Teorema de Lagrange}}:

\begin{align*}
&\text{(1) } H < G &&\Rightarrow && \lvert G \rvert = \lvert H \rvert \cdot [G : H] \\
&\text{(2) } K < H &&\Rightarrow && \lvert H \rvert = \lvert K \rvert \cdot [H : K] \\
&\text{(3) } K < G &&\Rightarrow && \lvert G \rvert = \lvert K \rvert \cdot [G : K]
\end{align*}
...
\textit{decorre da definição acima que $\vert G/H\vert = [G:H]$}.
