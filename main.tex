...
\[
x_p \overset{def}{=} x_1 x_2 ... x_p, \; p\in \mathbb{Z}^{*}_{+}
\]

\[
x_{p}^{-1} \overset{def}{=} x_{1}^{-1} x_{2}^{-1} ... x_{p}^{-1}, \; p \in \mathbb{Z}_{+}^{*}
\]

É evidente que $x_p,\; x_{p}^{-1} \in \bar{X}$. Assim como $x_p x_{p}^{-1} \in \bar{X}$, o que nos mostra que $\bar{X} \leq G$.
