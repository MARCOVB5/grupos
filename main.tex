...
\textit{Observação: note que $G'$ é de fato um subgrupo de $G$, uma vez que ele é, por definição, um subgrupo gerado.}
...
Seja $g \in G$ e tomemos $q = xyx^{-1}y^{-1} = [x,y], \; x,y\in G$.
...
Mostremos que $\mathcal{I}_g$ é de fato um automorfismo de $G$.

Mostremos, primeiramente, que $\mathcal{I}_g$ é um homomorfismo de grupos.

Sejam $x, y \in G$, temos:
...
Temos ainda também que, se definirmos $y \overset{def}{=} \mathcal{I}_g(x)$:

\[
\mathcal{I}_g(x) = y = gxg^{-1} \iff g^{-1}yg = x
\]

Logo, $\mathcal{I}_g$ é sobrejetiva.

Como $\mathcal{I}_g : G \to G$, concluímos que $\mathcal{I}_g$ é um automorfismo de $G$.
...
Mostremos, primeiramente, que para $y_1, y_2 \in \operatorname{Inn}(G), \; y_1\circ y_{2}^{-1}\in \operatorname{Inn}(G)$, onde $\circ$ é a composição usual de funções.
...
Ora, como $y_1, y_2 \in \operatorname{Inn}(G)$, podemos escrevê-los como:

\[
y_1(x) = g_1xg_{1}^{-1},\; \forall x \in G
\]

\[
y_2(x) = g_2xg_{2}^{-1}, \; \forall x \in G
\]

Temos:

\[
y_1\circ y_{2}^{-1} (x) = y_1(y_2(x)^{-1}) = g_1(g_{2}^{-1}xg_2)g_{1}^{-1} = (g_1g_{2}^{-1})x(g_2g_{1}^{-1}) \in \operatorname{Inn}(G)
\]

Logo, concluímos que $\operatorname{Inn}(G) \leq \operatorname{Aut}(G)$.

Provemos agora que para $(y, a)\in \operatorname{Inn}(G) \times \operatorname{Aut}(G), \; a\circ y\circ a^{-1} \in \operatorname{Inn}(G)$, isto é:

\[
a\circ y\circ a^{-1} = \mathcal{I}_h,\; \; \mathcal{I}_h\in \operatorname{Inn}(G)
\]

Temos:

\[
(a\circ y\circ a^{-1}) (x) = a(y(a^{-1}(x))) = a(ga^{-1}g^{-1}) = a(g)xa(g)^{-1} \in \operatorname{Inn}(G)
\]
...
\begin{proposition}
Seja $G$ um grupo e $H\leq G$, logo:

\[
H\trianglechareq G \Longrightarrow H \trianglelefteq G.
\]
\end{proposition}
...
Seja $H\trianglechareq G$, sabemos que:

\[
\forall \varphi \in \operatorname{Aut}(G), \; \varphi(H)\subseteq H
\]

Sabemos também que $\operatorname{Inn}(G) \subseteq \operatorname{Aut}(G)$, logo:

\[
(\varphi \vert_{\operatorname{Inn(G)}}) (H) = gHg^{-1} \subseteq H
\]

Deduzimos diretamente que:
