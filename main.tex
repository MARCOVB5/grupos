...
Sejam $x,y \in G$, sabemos que:

\[
xyx^{-1}y^{-1} \in G' \iff xy(yx)^{-1} \in G' \iff xyG' = yxG' \iff \bar{x}\bar{y}=\bar{y}\bar{x}, \; \bar{x},\bar{y}\in G^{ab}
\]
...
\[
xyH = yxH \iff xy(yx)^{-1} \in H \iff xyx^{-1}y^{-1} \in H \Longrightarrow \; \text{Absurdo!}
\]

Sabemos que $xyx^{-1}y^{-1}\in G'$, mas $G' \nsubseteq H$.

Deduzimos, portanto, que $G' \subseteq H$.
...
\[
\varphi_1(x) = g_1xg_{1}^{-1}
\]

\[
\varphi_2(x) = g_2xg_{2}^{-1}
\]

Temos:

\[
\varphi_1\circ \varphi_{2}^{-1} (x) = \varphi_1(\varphi^{-1}_2(x)) = g_1(g_{2}^{-1}xg_2)g_{1}^{-1} = (g_1g_{2}^{-1})x(g_2g_{1}^{-1}) = (g_1g_{2}^{-1})x(g_1g_{2}^{-1})^{-1} \in \operatorname{Inn}(G)
\]

Logo, concluímos que $\operatorname{Inn}(G) \leq \operatorname{Aut}(G)$.

Provemos agora que para $(\varphi, \phi)\in \operatorname{Inn}(G) \times \operatorname{Aut}(G), \; \phi \circ \varphi \circ \phi^{-1} \in \operatorname{Inn}(G)$. Temos:
...
Sabemos que $K\trianglechareq H \implies K \trianglelefteq H$.

Como $H \trianglelefteq G$ e a relação de normalidade é evidentemente transitiva, temos:
...
Definimos \textbf{produto direto de $\mathfrak{G}$} como sendo o produto cartesiano $G_1 \times \ldots \times G_n$ com a operação: 
...
Temos também, de forma evidente, que o elemento neutro e o elemento inverso de $(x_1,\ldots,x_n)$ são dados por, respectivamente:
...
\begin{theorem}
\color{red}
Sejam $G$ e $G_1, \ldots, G_n$ grupos. Temos que:
