...
\begin{enumerate}
	\item Primeiramente, notamos que $e$ e $\hat{x}$ são únicos, uma vez que:
...
Seja a operação "$\odot$" definida da seguinte maneira em $\mathbb{Z} / n\mathbb{Z}$, onde $n\in \mathbb{Z}$:

\[
\odot: 
\begin{array}{c}
\mathbb{Z} / n\mathbb{Z} \bigtimes \mathbb{Z} / n\mathbb{Z} \to \mathbb{Z} / n\mathbb{Z} \\
(\bar{a}, \bar{b}) \mapsto \bar{a} \odot \bar{b} = \bar{a\cdot b}
\end{array}
\] 

onde "$\cdot$" indica a operação usual de produto nos inteiros.

Temos que $(\mathbb{Z}^* / p\mathbb{Z}, \odot)$, onde $p$ é um número primo, é um grupo abeliano.

\end{example}

\textbf{Demonstração:}

Para fins de simplificação de notação, nesta demonstração iremos utilizar $\mathbb{Z} / n\mathbb{Z} \overset{def}{=} \mathbb{Z}_n$

Comecemos demonstrando que de fato temos:

\[
\forall (\bar{a}, \bar{b}) \in \mathbb{Z}_p \times \mathbb{Z}_p \implies \bar{a} \odot \bar{b} \in \mathbb{Z}_p
\]
...
\begin{proposition}
A partir das notações acima temos que:
\[
\langle X \rangle = \{a_1 a_2 ... a_n : a_i \in X \; \; \text{ou} \; \; a^{-1}_{i} \in X, \; n \geq 1 \}
\]
\end{proposition}
...
\begin{proposition}
A partir das notações acima temos que $\langle X \rangle$ é o menor subgrupo de $G$ que contém $X$.
\end{proposition}
...
\textit{Observações:}
    \begin{itemize}
        ...
        \item \textit{Quando não houver confusão possível, podemos denominar as classes laterais à esquerda/direita de $H$ em $G$ que contém $x$ como simplesmente: classe lateral à esquerda/direita de $G$}.
    \end{itemize}
...
\end{definition}
{\color{red}
\textit{Observação}: note que o número de classes laterais à direita de $G$ é igual ao número de classes laterais à esquerda de $G$ (por mais que as classes laterais sejam diferentes). 
...
\begin{definition}
	Seja $G$ um grupo. O subgrupo $H$ de $G$ é chamado de \textbf{subgrupo normal de $G$} se:
	\[
	\forall (g, h) \in G \times H, \; \; ghg^{-1} \in H.
	\]
	Denotamos:
	\[
	H \triangleleft G.
