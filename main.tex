...
\textit{Observação:} de forma geral, é evidente que se $H < Z(G)$, então $H \trianglelefteq G$.

\begin{example}
Em particular, percebemos que se $G$ é um grupo abeliano, então todos seus subgrupos são subgrupos normais de $G$.
\end{example}
...
\begin{proposition}
	Seja $G$ um grupo e $Z(G)$ seu centro. Se $G/Z(G)$ for cíclico, então $G = Z(G)$.
\end{proposition}
...
Seja $x$ um gerador de $G/Z(G)$, isto é, $\langle x \rangle = G/Z(G)$.

Portanto, sendo $f$ um número inteiro, podemos escrever $x$ como $x = g^fz', \; (g, z') \in G\times Z(G)$.

Isso ainda equivale a escrever: $g = x^kz, \; $


\[
x_1 \overset{=}{def} g_{1}^{k_1}z_1, \; (g_1, z_1) \in G\times Z(G) 
\]

\[
x_2 \overset{=}{def} g_{2}^{k_2}z_2, \; (g_2, z_2) \in G\times Z(G)
\]
