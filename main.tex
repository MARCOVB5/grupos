...
\textit{Note que $A$ e $B$ são de fato subgrupos de $S_3$, entretanto, tal fato não será mostrado pois não cabe ao nosso objetivo final.}
...
\begin{definition}
Definimos o \textbf{núcleo de um homomorfismo $\varphi$ o subgrupo normal de $G$, $\ker \varphi$, tal que:}

\[
\ker \varphi \overset{def}{=} \{\varphi(x) = e_\mathcal{G}, \; x\in G\}.
\]
\end{definition}

\textbf{Demonstração:}

Mostremos que $\ker \varphi$ é de fato um subgrupo normal de $G$.
...
\begin{definition}
Definimos a \textbf{imagem de um homomorfismo $\varphi$ o subgrupo de $G$, $\operatorname{Im} \varphi$, tal que:}
