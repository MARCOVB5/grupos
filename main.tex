...
\begin{definition}
\color{red}
Sejam $n\in \mathbb{Z}^{*}_{+}$ e $\mathfrak{G} = \{G_1, \ldots, G_n \}$ uma família de grupos.

Definimos \textbf{produto direto de $\mathfrak{G}$} como sendo o conjunto $G_1 \times \ldots \times G_n$ munido da operação: 
...
\begin{proposition} 
\color{red}
A partir da \hyperlink{def171}{\textbf{Definição 1.7.1}}, temos que o produto direto de $\mathfrak{G}$ é um grupo.
\end{proposition}

{\color{red}
\textbf{Demonstração:}

Seja $n\in \mathbb{Z}_{+}^{*}$.
...
\[
e = (e_1, \ldots, e_n) \in \prod_{i=1}^{n} G_i
\]

Onde $e_n$ representa o elemento neutro do $n$-ésimo grupo do produto direto.

\[
x^{-1} = (x_{1}^{-1},\ldots, x_{n}^{-1})\in \prod_{i=1}^{n} G_i
\]

Onde $x_{n}^{-1}$ representa o elemento inverso para $x_n$ no $n$-ésimo grupo do produto direto.

Temos também que o produto direto de $\mathfrak{G}$ é evidentemente associativo, uma vez  que $G_1, \ldots, G_n$ são grupos.
...
\begin{theorem}
Sejam $n\in \mathbb{Z}_{+}^{*}$ e $G,\;G_1, \ldots, G_n$ grupos. Temos que:

\[
G \cong \prod_{i=1}^{n}G_i \iff \forall i\in \llbracket 1,n \rrbracket,\; \exists H_i \leq G \; \text{e} \; H_i\cong G_i, \; \text{tal que:}
\]

\begin{enumerate}
	\item $G = H_1\ldots H_n$;
	\item $H_i \trianglelefteq G,\; \forall i\in \llbracket 1,n\rrbracket$;
	\item $H_i \cap H_1\ldots H_{i-1}H_{i+1}\ldots H_n = \{e\},\; \forall i\in \llbracket 1,n\rrbracket$.
\end{enumerate}

Caso $G$ e $H_1,\ldots ,H_n$ satisfaçam as condições acima, denotamos:

\[
G \overset{def}{=} \bigoplus_{i=1}^{n} H_i.
\]
