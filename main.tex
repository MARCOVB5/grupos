...
$(ii)$ $\exists t \in \mathbb{Z}_{+}^{*} : \alpha ^t = e$, onde $t = \min{\{k \in G : k > 0\}}$ e $\mathcal{O}(\alpha) = t$.
...
Sabemos, a partir da \hyperlink{defglinha}{\textbf{Definição1.3.5}} que $G' \leq G$.

Seja $g \in G$.

Definimos ainda, sem perda de generalidade, $g'\in G'$ tal que $g' = xyx^{-1}y^{-1}$.

Temos, portanto:

\[
gg'g^{-1} = g(xyx^{-1}y^{-1})g^{-1} = (gxg^{-1})(gyg^{-1})(gx^{-1}g^{-1})(gy^{-1}g^{-1})
\]

Tomando $k = gxg^{-1}$ e $l = gyg^{-1}$, deduzimos que:

\[
gg'g^{-1} = klk^{-1}l^{-1} \in G'
\]

Portanto, $G' \trianglelefteq G$.
